...
class TestSelectionAndHelpers(unittest.TestCase):
    def test_clean_tier_with_no_primary_match_returns_first_match(self):
        service = make_markdown_service(
            [
                StubExtractor(
                    'a', 'text-a', confidence=0.9, file_types=(FileType.PDF.value,)
                ),
                StubExtractor(
                    'b', 'text-b', confidence=0.5, file_types=(FileType.PDF.value,)
                ),
            ],
        )
        with mock.patch(
            ...
            result = service.extract(b'x', FileType.PDF.value)
    ...
    def test_completeness_failure_appends_tail_flag(self):
        ...
        service = make_markdown_service(
            [
                StubExtractor(
                    'a', 'apple', confidence=0.9, file_types=(FileType.PDF.value,)
                ),
                StubExtractor(
                    'b',
                    'zebra giraffe lion',
