...
class TestTxtExtractor(unittest.TestCase):
    def test_utf8(self):
        result = TxtExtractor().extract('مرحبا'.encode('utf-8'), FileType.TXT)
        self.assertEqual(result.markdown, 'مرحبا')
        self.assertEqual(result.confidence, 1.0)

    def test_utf16(self):
        result = TxtExtractor().extract('hello'.encode('utf-16'), FileType.TXT)
    ...
    def test_falls_back_to_latin1(self):
        ...
        result = TxtExtractor().extract(b'\xa3', FileType.TXT)
